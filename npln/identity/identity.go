...
type Resolver struct {
	...
	// revoked reports whether an nx2 payload has been revoked. Injected so the
	// denylist lives with the rest of the deployment's account integration
	// (revoked.go), exactly as it does in every NEX game server.
	revoked func(payload string) bool
}
...
type Options struct {
	...
	LookupNSA func(nsaHex string) (uint64, error)
	// Revoked reports whether an nx2 token payload ("pid.username.expiry") has
	// been revoked. A valid signature is not enough: a leaked token keeps
	// verifying until the shared secret is rotated, which would log out every
	// player, so each server carries a denylist. Optional; nil accepts every
...
func (r *Resolver) pidFromNexToken(tok string) (uint64, bool) {
	...
	// A correct signature is NOT sufficient. The payload leaked by the 1.6.5
	// release verifies perfectly and is revoked by configuration instead — the
	// same check every NEX game server performs (see revoked.go).
	if r.revoked != nil && r.revoked(string(raw)) {
		return 0, false
	}
